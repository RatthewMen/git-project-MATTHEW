...
5.0 (10/14/2025)
    #1
commitTesterFlatFiles()
Modifier and type: static void
Creates a flat file (flat.txt), adds it to staging, and commits it with a given author and message to test the commit process.

    #2
commitTesterEmptyIndex()
Modifier and type: static void
Deletes the git directory, reinitializes it, and attempts a commit with an empty index to verify correct behavior when no files are staged.

    #3
addTesterBlobMade()
Modifier and type: static void
Adds ZidFolder\dpeng.txt and loops through the objects folder to confirm that the blob file 5ea38cf69bccefb8cb061592e068d30d4869b60c was created.

    #4
addFileDupeFile()
Modifier and type: static void
Adds the same file twice to test how duplicate entries in the index are handled.

    #5
addFileContentsChecker()
Modifier and type: static void
Adds ZidFolder\dpeng.txt and checks the index file to ensure it contains the correct hash and path entry.

    #6
addTesterNotRealDirectory()
Modifier and type: static void
Tests add() with a directory path instead of a file to confirm proper handling of invalid input.

    #7
addTesterNotRealPath()
Modifier and type: static void
Tests add() with a nonexistent path to verify correct error handling.

    #8
deleteZidFolder()
Modifier and type: static void
Deletes the ZidFolder directory and all of its contents recursively for cleanup after testing.

    #9
createZidFolder()
Modifier and type: static void
Creates a folder named ZidFolder with two text files (zid.txt and dpeng.txt) and a subfolder ZidFolder2 containing zid2.txt for testing setup.

    #10
InitTesterPaths()
Modifier and type: static void
Checks that the GitDirectory paths (git, objects, HEAD, index) are correctly initialized.

    #11
InitTesterRepetition(int times)
Modifier and type: static void
Runs init() multiple times to ensure consistent and safe reinitialization behavior.

    #12
InitTesterDeleteHead()
Modifier and type: static void
Deletes the HEAD file and reinitializes Git to verify that the file is recreated.

    #13
InitTesterDeleteIndex()
Modifier and type: static void
Deletes the index file and reinitializes Git to confirm it is recreated correctly.

    #14
InitTesterDeleteObjects()
Modifier and type: static void
Deletes the objects directory and runs init() again to confirm it is properly recreated.

    #15
InitTesterDeleteGit()
Modifier and type: static void
Deletes the entire git directory and reinitializes to verify the full repository structure is rebuilt.


4.3 (10/12/2025):
Bugs and fixes:
The method didn't properly sort the list of lines of blobs and trees before using it. Now, the lines with the most slashes are at the top and it alphabetically sorts downward. Also, the working list isn't updated throughout every use of the method void IndexTreeGenerator(ArrayList<String> wList). The working list wasn't in the objects folder of git. Some of the code wasn't very clear and redundant. The README wasn't updated. The tester called on the wrong classes for some methods. The tester was lacking some methods that it should've tested. I made tester methods for every method in the GitDirectory, Hashing, and RandomFiles classes. I tested all the methods in the Git class too. The remakeHashmap() method didn't work, since he didn't clear the HashMap before making it. I fixed that too. I fixed Matthew's hashmap, as he had the key be the SHAhash and the value be the filepath. This is incorrect, as blobs and trees can have the same hash value, making the Hashmap useless in a sense. Switching this was a big help, as now, the blobs can have the same content in them, but no two directory paths to a blob will be the same. The content in the blob will just be overwritten. I also removed an extra new line at the end of the index file and subtree files. Please give me as much bonus points as possible. Need them and each is very much appreciated. 
